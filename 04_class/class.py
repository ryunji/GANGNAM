...
class Person:
    ...
    def greeting(self):
        return 'hello this is python' #꼭 자신을 첫번째 인자로 넘겨주어야 함.
    
# 클래스 속성 : 클래스 자체에 속하는 변수로, 모든 인스턴스가 공유하는 속성이다.
# 해당 클래스로 생성된 인스턴스가 공유하는 속성.
...
# 메소드 : 클래스 내부에 정의된 함수로, 인스턴스의 데이터를 조작하거나 동작을 정의한다.
    ...
    #        객체를 통해 접근 시 호출되는 메소드의 첫 번째 인자로 항상 self를 넘겨 주어야 한다.     
...
class Student:
    ...
    def display_info(self):
        print(f'student name : {self.name}')   #바로 변수에 접근 가능
...
print("전역 : ", variable)      #모듈은 .py파일을 말함.
...
# 설정 값을 전역으로 유지하고, 여러 함수에서 이 값을 변경하거나 참조할 때 유용하다.
...
# 비공개 속성에 직접 접근 시도
# __(언더스코어 2개)로 시작하는 속성은 완전한 의미의 private는 아니지만, 어느 정도 보호되는 속성으로 취급 함.
# 이름 이름 장식(name mangling) 이라고 함.
