...
# Request : HTTP 요청 객체를 포함.
# depends 키워드로 인해 세션 객체가 먼저 생성 됨
@app.get("/")
async def home(request:Request, db : Session = Depends(get_db)):

    # 데이터베이스에서 Todo 모델을 조회하고, id 기준으로 내림차순 정렬
...
async def add(request:Request, todo_id:int, db:Session = Depends(get_db)):
    # 데이터베이스에서 Todo 모델을 가져와서, id가 todo_id와 일치하는 첫 번째 항목을 가져옴
...
async def delete(request:Request, todo_id:int, completed:bool = Form(False), db:Session = Depends(get_db)):
